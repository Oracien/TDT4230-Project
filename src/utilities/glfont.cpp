...
Mesh generateTextGeometryBuffer(std::string text, float characterHeightOverWidth, float totalTextWidth) {
    ...
    for(unsigned int i = 0; i < text.length(); i++)
    {
        ...
        int charNum = (int) text.at(i);

        float baseUCoordinate = (charNum / 128.0f);
        mesh.textureCoordinates.at(4 * i + 0) = {baseUCoordinate, 0.0f};
        mesh.textureCoordinates.at(4 * i + 1) = {baseUCoordinate + (1.0f/128.0f), 0.0f};
        mesh.textureCoordinates.at(4 * i + 2) = {baseUCoordinate + (1.0f/128.0f), 1.0f};
        mesh.textureCoordinates.at(4 * i + 3) = {baseUCoordinate, 1.0f};
